...
- **No usar `<Text>` de drei sin prop `font` local** — troika-three-text cae en `unicode-font-resolver` y baja la tipografía EN RUNTIME desde `cdn.jsdelivr.net` (2 JSON + 2 `.woff`). Usar `font="/fonts/Poppins-Bold-subset.ttf"` (subset Latin-1 de 13,9 KB; el comando `pyftsubset` que lo genera está en el comentario de `PageCube.tsx`). `carousel-3d.tsx` se quitó troika por esto mismo.
