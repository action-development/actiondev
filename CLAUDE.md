...
**Plaza de reseñas** (`src/app/resenas/` + `src/components/canvas/PlazaScene.tsx` + `canvas/plaza/*` + DOM en `src/components/plaza/*`): mismo fondo oscuro que el resto de la web (`--background` #080808, acento lima; antes era blanca por decisión del cliente, ya revertida). Cielo/niebla/suelo plano en `PLAZA_PALETTE`, DOM con tokens semánticos. **Arrastrar**: click mantenido (> 6 px de movimiento o > 400 ms) agarra al muñeco — estado `held` en `PlazaDoll` (brazos en alto agitándose, piernas colgando) y sigue al puntero por un plano VERTICAL de cara a la cámara (`PlazaWorld`, listeners en `window`): izquierda/derecha = lado, arriba/abajo = ALTURA, la profundidad no cambia. La PROFUNDIDAD (acercar/alejar) es el tercer eje y tiene tres vías, todas contra la misma función (`canvas/plaza/drag-depth.ts`, `pushDepth` empuja el plano de arrastre a lo largo de la vista, acotado por `DEPTH_RANGE` y `MAX_RADIUS`): **rueda** del ratón (`DEPTH_STEP`), **↑ ↓ / W S** mientras se lleva el muñeco (`DEPTH_KEY_SPEED`; arriba = al fondo, mismo lenguaje que ▲▼ del hero) y **Mayús + ratón**, que convierte el eje vertical del puntero en profundidad (`DEPTH_PER_PX`, cursor `ns-resize`). Tres y no una porque la rueda solo se puede girar con el botón pulsado y ese gesto no existe en un trackpad — era el motivo de que el eje pareciera no estar. Al entrar o salir del modo Mayús hay que reenganchar el NDC vertical **en el mismo evento** (`setDepthMode`) y recalcular el desfase en el frame siguiente (`rebase`): si no, el muñeco salta media pantalla. `pushDepth` acota la distancia CON SIGNO sobre el eje de vista, no el radio: con el radio, un tirón fuerte cruzaba la cámara y el muñeco reaparecía detrás del visitante. Al soltarlo cae con gravedad y ahí su sitio pasa a ser su nueva casa (`DollRuntime.home`). Un click sin arrastre abre la ficha al SOLTAR, no al pulsar. **Legibilidad del arrastre** (el fondo es plano y la cámara va casi a ras: subir y alejarse se ven igual): anillos concéntricos en el suelo (`FloorGrid` en `PlazaRoom`, se apagan con la niebla), y mientras está en el aire un cordón vertical + aro de aterrizaje en lima (`geos.tether`/`geos.ring` en `PlazaDoll`, altura vía `liftRef`) más la sombra de contacto, que se encoge al subir. No oscurecer `PLAZA_PALETTE.floorNear`: sin sombra visible no hay forma de saber dónde está un muñeco en el aire. `/resenas?quieto=1` congela paseo y órbita (patrón `?hora=` del hero): la plaza viva no se puede apuntar desde un test —R3F solo recalcula el hover al mover el puntero, así que el cursor "grab" se queda obsoleto cuando el muñeco se aparta— ni da dos capturas iguales. Úsalo en e2e y en snapshots. Contrato compartido en `canvas/plaza/plaza-config.ts` (paleta, medidas `DOLL`, PRNG determinista por id → el mismo cliente tiene siempre el mismo muñeco, requisito para snapshots). 0 assets: caras, cielo y suelo son `CanvasTexture` en runtime. Copiamos el lenguaje visual (proporciones, plaza, comportamiento), nunca marcas/nombres/assets de Nintendo — no usar la palabra "Mii" en copy ni código público.
