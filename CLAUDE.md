...
| `.calm-surface` | El MISMO cuadrado con el proyector bajado. **Sin uso** desde que `/contact` es la calle 3D (2026-09-24); se conserva por si una página de lectura lo pide |
...
|------|--------|-----------|
| `/` | Home | Pantalla de inicio "rollo videojuego": SOLO el hero "La Grúa" a viewport completo (`h-[100dvh]`), sin scroll, sin Lenis, sin footer. Cargar un contenedor en el barco → wipe radial → `router.push` a su ruta. **Texto indexable oculto** al inicio de `<main>` (`app/page.tsx`): `<h1>` = `BRAND.tagline`, `BRAND.shortDescription` y un `<nav aria-label="Servicios">` con `/servicios` + todas las landings (`landings.ts`); `sr-only` y visible solo con `focus-within` (patrón del "Saltar al contenido"). Es la ÚNICA vía de enlace a las landings desde una página navegable: **no enlazarlas desde Header, Footer ni "Ir sin jugar"** (decisión del cliente) |
| `/projects` | Sala recreativa | Pasillo 3D en primera persona: una recreativa por proyecto a cada lado y, al fondo, la puerta «TRABAJEMOS JUNTOS» que lleva a `/contact`. Clic en una máquina = la cámara se ACOPLA a su pantalla y la ficha del proyecto se enciende dentro (no navega). Pantalla completa como `/resenas` (Header, sin Footer ni Lenis) — ver nota abajo |
...
| `/contact` | Calle Colón 20 | El portal REAL de la oficina (C/ Colón 20, Vigo) en 3D, detrás de la puerta del fondo de la recreativa. Cabina = WhatsApp, buzón = email, portero automático = "llámame tú". Los canales están SIEMPRE en el HUD sin tener que jugar. Pantalla completa como `/resenas` (Header, sin Footer ni Lenis). Ver nota abajo |
...
| `/tienda-online-vigo` | Landing SEO | ídem — ecommerce (Shopify / a medida) |
| `/agencia-desarrollo-web-galicia` | Landing SEO | ídem — intención «agencia», distinta de la de aplicaciones para no canibalizar |

**Sala recreativa** (`app/projects/ArcadePage.tsx` + `canvas/ArcadeScene.tsx` + `canvas/arcade/*` + HUD en `arcade/ArcadeHud.tsx`): pasillo RECTO hacia -Z con dos filas de máquinas mirando al camino central y la puerta al fondo (decisión del cliente: primera persona, los 32 proyectos, día y noche). Contrato en `canvas/arcade/arcade-config.ts` (medidas `HALL`/`CABINET`/`DOOR`/`WALK`; `buildMachines` pone los destacados junto a la entrada, alternando izquierda/derecha; color del lateral por categoría).
- **Cámara sobre raíles**: solo cambia `z` al andar; la cabeza mira al frente o a una fila. ↑/W ↓/S andan (con inercia; también rueda y, en táctil, arrastre vertical); ←/A →/D giran izquierda ← centro → derecha (en táctil, barrido horizontal); Enter/Espacio o clic (en la máquina o en la ficha de abajo) ACOPLAN. Mirando a una fila sin andar, un **imán** te planta delante de la máquina enfocada (sin él, en la entrada la pantalla quedaba de canto). Sin colisiones ni navegación libre: no hay forma de perderse.
- **Acople (decisión del cliente: elegir una máquina NO navega)**: `ArcadePage` guarda `selected`; `ArcadeWorld` planta la cámara sobre la normal de la pantalla a la distancia de `dockPosition` (`arcade-config.ts`) para que el tubo ocupe `DOCK.fill` (0,6) del viewport, y avisa con `onDockedChange` al llegar. La ficha es DOM (`arcade/ArcadeScreen.tsx` + module): se dimensiona con la MISMA fracción y el mismo 4:3 (`min(60vw, 80vh)`), así cae encima del tubo al píxel sin proyectar esquinas — si se toca `fill`, el `fov` de la cámara o el aspecto de `CABINET.screen`, cambian los dos lados a la vez. Dentro: contador, categoría, título, vídeo/captura, "Qué nos pidieron / Qué conseguimos" (`lib/project-case.ts`, compartido con la ficha `/projects/[slug]`) y enlaces "Ver web" / "Ficha completa". **Salir tiene que ser obvio**: botón `HoloButton` solid "Volver al pasillo" con la tecla `Esc` impresa, abajo al centro (donde estaba la ficha del pasillo); Esc, ↓/S o Retroceso; y clic fuera del tubo. Flechas laterales con el NOMBRE de la vecina y ←/→ pasan a la máquina de al lado de la MISMA fila (`neighborMachine`: mirando a la fila izquierda, la derecha es el fondo del pasillo). Acoplado, el pasillo está en `paused` (las teclas son de la pantalla), el HUD del pasillo se retira y el vídeo 3D se para (la ficha trae el suyo). Al soltar, la cámara vuelve al raíl con suavizado, frente a esa misma máquina.
- **Puerta y salida** (`canvas/arcade/ArcadeDoor.tsx`): vano recortado de verdad en la pared del fondo (`ShapeGeometry` en U en `ArcadeWalls`, UV en metros), forro de acero con fondo `DOOR.reveal`, tubo de neón por el canto y halo en MARCO (textura con el hueco vacío: un halo radial aditivo sobre el vano saturaba el lima de dentro a amarillo). Dos hojas de cristal con perfil, zócalo y barra antipánico, bisagra al fondo del vano; se entreabren (`DOOR.ajarAngle`) al apuntarlas. Detrás, la **sala de neón**: caja `#080808` con líneas de fuga y arcos lima (una `InstancedMesh`) que acaba en una pared de 10 lamas = la persiana del sitio en 3D. Llegar a `WALK.doorZ` (o clic en la puerta = andar solo hasta ella) dispara la salida (`EXIT` en `arcade-config.ts`, bucle en `ArcadeWorld`): las hojas se abren, la cámara entra con un golpe de `fov` y para en `EXIT_CAMERA_Z`, a los ojos, mirando de frente y con `CAMERA_FOV` exacto, donde la pared de lamas llena el viewport — alto `2 · eyeHeight`, hueco de `NEON_ROOM.gapPx` píxeles de pantalla como `ui/Blinds`. Ese frame ES la persiana cerrada (medido: ±1 px, lima 201/255/0): al frame siguiente `navigate("/contact", { covered: true })` la pone cerrada SIN animar y solo se anima la apertura sobre `/contact`. Si se toca `CAMERA_FOV`, `BLIND_COUNT`, el hueco de las lamas o `WALK.eyeHeight`, el relevo deja de casar. **Antiparpadeo**: todo lo lima fino (tubo del vano, líneas y arcos de la sala) se reescala cada frame para no bajar de `MIN_PX` = 1,5 px en pantalla, y el hueco de las lamas es SIEMPRE `gapPx` en pantalla (a 30 m medía 0,3 px y se deshacía en una trama que palpitaba); halo, charco y barras llevan `polygonOffset` (estaban a milímetros de su superficie y hacían z-fighting). No volver a poner líneas de grosor fijo en metros ni filetes finos en las hojas. Durante la salida la página retira Header y HUD. Con `prefers-reduced-motion` no hay coreografía: persiana normal. La posición se guarda en `sessionStorage` (`action-arcade-position`) al acoplar una máquina (por si desde su pantalla se abre la ficha completa) y se borra al salir por la puerta — si no, al volver de `/contact` se reaparecería en la puerta y rebotaría.
- **Rendimiento**: piezas del mueble instanciadas (una llamada de dibujo por pieza), las 32 marquesinas son UNA malla fusionada sobre un atlas, zona de click = `InstancedMesh` invisible (`instanceId` = máquina). Lo único por máquina es la pantalla. Pantallas: demo procedural (`getAttractTexture`) → captura (`TextureLoader`, fuera de Suspense, recorte "cover" 4:3) → **vídeo solo de la enfocada**, con 320 ms de retardo para no bajar un `.webm` por máquina al andar, y nunca con `prefers-reduced-motion`. Los proyectos con `PLACEHOLDER_IMAGE` se quedan en demo con su nombre.
- **Mueble** (`canvas/arcade/cabinet-geometry.ts`): TODO sale del perfil lateral `PROFILE` de `arcade-config.ts` (silueta clásica, puntos `[z, y]` antihorarios). Cuerpo y laterales son ese perfil extruido; perfil de goma, vinilo, bisel, panel de mandos, rejilla y marquesina se montan sobre su tramo con `segmentFrame`. Retocar la silueta recoloca todo. Mandos en `PANEL_CONTROLS` (una sola fuente para la serigrafía y las piezas). Tubo CRT curvo (`crtGeometry`) con capas de barrido, viñeteado y reflejo. Lateral = color de categoría casi a negro (`mutedSide`); el vivo va en goma y vinilo. Sombra de contacto bajo cada mueble.
- **Sala**: `ArcadeWalls` (friso, yeso, mural de luz negra, riel de bombillas en persecución con portalámparas, SIN halo (decisión del cliente), pósters en caja de luz con juegos inventados que guiñan a la home/Vigo, neones con separadores y halo, moldura de luz con bañado, altavoces), `ArcadeCeiling` (forjado grecado, vigas en I, conducto con abrazaderas, bandeja de cables, focos con haz aditivo (shader propio `beamMaterial`: la opacidad cae hacia la silueta por el ángulo con la vista, y la niebla resta alfa en vez de sumar color) y charco, lucernarios) y `ArcadeFloor` (moqueta con fibra y `bumpMap`, capa de desgaste, balizas de pasillo con pulso hacia la puerta, umbral y felpudo). Texturas de sala en `hall-textures.ts`. Los haces y halos NO son luces reales (coste por píxel en todo): conos/planos aditivos. Única luz real añadida: `FocusLight`, una sola que se muda a la máquina enfocada.
- **0 assets de decorado**: moqueta, marquesinas, rótulo de neón y pantallas demo son `CanvasTexture` (`arcade-textures.ts`, singletons liberados al salir). El logo de la puerta es `logo.webp` pasado por canvas: la luminancia invertida se hace alfa y se tiñe de lima (el webp es tinta negra sobre blanco, sin alfa). Puerta, rótulo y logo van con `fog={false}`: son la CTA y se leen desde la entrada.
- **Día y noche** (`arcade-mode.ts`, `?hora=` como puerto y plaza): de día lucernarios encendidos, paredes claras y neón apagado; de noche neón lima y brillo de pantallas. Las máquinas están encendidas en los dos.
- **HUD**: tutorial con `ui/ControlSign` en dos pasos (`data-step` walk → pick → done; se da por hecho al enfocar la primera máquina), ficha de la enfocada (`data-testid="arcade-focus"`, botón que la acopla; testids de la pantalla: `arcade-screen`, `arcade-screen-close`, `arcade-screen-prev/next`) y «Ver lista» (`HoloButton` quiet sin `href`, `arcade-list-toggle`). La lista (`arcade-list`) va con `hidden` cerrada: fuera de la vista y del árbol accesible, pero los 32 enlaces siguen en el HTML para buscadores. Con la lista abierta el pasillo no escucha teclas (`paused`).
- **Sin regresión visual** (como `/resenas`): capturas y vídeo llegan cuando llegan. `?quieto` congela la mirada libre del ratón; los tests viven en `smoke.spec.ts`.
- Persiana de carga compartida con la plaza: `ui/SceneCurtain.tsx` (`testId` `plaza-curtain` / `arcade-curtain`).
- **Huérfanos pendientes de decisión**: `sections/Projects.tsx` (carrusel 3D + contenedor colgado) y `sections/ProjectsIndex.tsx` ya no tienen ruta; arrastran `effects/carousel-3d.tsx`, `canvas/HangingCargo.tsx`, `effects/card-background-preview.tsx` y `projects-hud.module.css`.

**Contacto — la calle** (`app/contact/ContactPage.tsx` + `canvas/StreetScene.tsx` + `canvas/street/*` + DOM en `contact/ContactHud.tsx` y `contact/CallbackForm.tsx`). **Decisión del cliente (2026-09-24): se revierte la "calma deliberada"** — la página plana no tenía el nivel del puerto, la recreativa y la plaza. La condición que NO se negocia: la conversión no depende del juego.
- **La escena** reproduce el portal real (foto de referencia del cliente, NO usada como textura: es de un directorio y lleva marca de agua). Cada parte en `canvas/street/facade/`: `WoodCladding` (CADA listón es una caja en un único `InstancedMesh`, ~700 instancias, tono por `instanceColor`, rastrel oscuro detrás — un plano con listones pintados se veía de cartón), `UpperFloors` (muro hecho de antepecho/machones/dintel, así los huecos tienen jamba de verdad; carpintería, vidrio, persianas a alturas distintas, alféizar, cornisas por forjado y coronación), `WhiteWall` (estuco, rótulo corpóreo separado del muro con su sombra, zócalo de sillería con albardilla, barandilla de anillos 3D, jardinera), `Portal` (pilastras, ascensor de acero cepillado, puertas de vidrio con perfilería y tiradores, granito pulido, focos con aro) y `StreetGround` (adoquín, losa, bordillo, calzada, macetones con ficus trenzado, bolardos, murete de ladrillo). Contrato de medidas en `street-config.ts`. 0 assets: todo `CanvasTexture` (`street-textures.ts`). Helpers en `street-geometry.ts`: `planarUV` (UV en metros por cara según la normal, para que la junta de la sillería siga de una pieza a otra), `leafBall` (copa orgánica: icosaedro abollado con normales suaves), `roundedBox`, `atlasPlane`, `merge`. Reflejos: `RoomEnvironment` + PMREM (procedural; los `preset` de drei están prohibidos), peso por modo en `envIntensity`.
- **Los tres objetos**, a nivel de producto: `PhoneBooth` (perfilería redondeada, caja de luz «TELÉFONO» con pictograma, zócalo cepillado, chapa lagrimada; teléfono con LCD lima, teclado de 12 teclas con número desde atlas, ranuras, cajetín, horquilla, auricular y cable blindado que se rehace mientras se descuelga), `Postbox` (lacado con `clearcoat`, bóveda, trampilla articulada que abre la carta, placa en relieve, horarios, cerradura, pie con brida) e `Intercom` (marco anodizado, frontal cepillado, cámara con lente e IR, tarjetas retroiluminadas de noche, pulsadores; el de Action con aro lima que se hunde al llamar). `Hotspot.tsx` pone baliza lima + etiqueta DOM (drei `<Html>` con `zIndexRange={[5, 0]}`, por debajo del HUD y del Header). Clic = el objeto se anima Y el canal se abre. `window.open` de WhatsApp va SÍNCRONO dentro del clic (fuera de un gesto lo bloquea el navegador). Sin marcas: nada de logotipos de operadoras ni de Correos.
- **Acercamiento al apuntar**: hover en un objeto (o en su canal del HUD) → la cámara se planta delante de él (`FOCUS` en `street-config.ts`) y lo deja por encima del HUD (`focusScreenY`); al salir vuelve a la acera. Posición y mirada se suavizan hacia el destino (`focusLambda`). Anti-bucle: la salida del hover se retrasa 320 ms y la zona de hover crece ×1,7 mientras el objeto está enfocado — sin eso el objeto se movía bajo el ratón en pleno acercamiento, "salía" y la cámara oscilaba. El portero y el macetón izquierdo están separados a propósito: la copa lo tapaba al enfocarlo.
- **HUD** = la conversión: panel `.holo-surface.holo-solid` abajo al centro con dirección, titular, las dos tarjetas `[data-channel]` (enlaces `wa.me` / `mailto:` de `data/socials.ts`, como antes), el "¿Prefieres que te contactemos?" (monta `CallbackForm` —el único `<form>`, solo al abrirlo, y enfoca el teléfono— que escribe en `leads`) y los chips de IA. HUD y calle enlazados en los dos sentidos: apuntar un canal resalta su objeto (`street-tag-*[data-active]`) y al revés; usar un canal desde el HUD también anima el objeto. Pista `ControlSign` arriba hasta el primer uso.
- **Cámara**: fija en la acera de enfrente, paralaje de ratón y entrada de 1,8 s. Con pantallas estrechas retrocede (`cameraDistance`) para que cabina y buzón quepan, baja la mirada (`targetDrop`) para que el portal quede por encima del HUD, y la niebla se desplaza con ella (con niebla fija, en móvil la calle quedaba entera dentro de la niebla).
- **Luz**: día/noche por hora local o `?hora=` (`street-mode.ts`). Solo el sol proyecta sombra (misma receta que la plaza: ortográfica recalculada a mano). De noche: una luz puntual en el soportal, la farola fuera de plano (spot) y los haces de los focos como conos aditivos, no luces. `?quieto` congela paralaje, entrada y balizas.
- Sin regresión visual (escena 3D, como `/resenas` y `/projects`); lo cubren `smoke.spec.ts` y `sections.spec.ts`. `#contact` es el panel del HUD. Persiana: `SceneCurtain` con `testId` `street-curtain`.

**Atajo de contacto** (`ui/ContactPopup.tsx`, montado en el layout raíz DENTRO de `PageTransition` porque lee su `busy`): popup para el visitante con prisa que no quiere aprender a jugar. `<dialog>` nativo con `showModal()` (foco atrapado, Esc, clic en el backdrop), render solo cuando se abre. **Disparadores** (decisión del cliente): intención de salida (`mouseout` con `relatedTarget` nulo y `clientY <= 0`) o implicación sin conversión (40 s de pestaña visible, o segunda ruta + 15 s en ella); nada en los primeros 8 s y el disparador por tiempo NO actúa en `/` (se está jugando; la salida sí). **Nunca** en `/contact` ni `/legal/*`, con el banner de cookies sin decidir, con la persiana en marcha, más de una vez por sesión (`sessionStorage` `action-contact-popup-shown`), en los 7 días siguientes a cerrarlo ni si ya contactó por WhatsApp/email desde CUALQUIER parte del sitio (`localStorage` `action-contact-popup` = timestamp o `"converted"`; ambas claves documentadas en `/legal/cookies`). Contenido: pregunta "¿Qué quieres construir?", 3 servicios como radios (apps / web app / web premium) que prerredactan el WhatsApp, CTA sólido WhatsApp, email como enlace secundario y los chips de IA como terciario. Imagen `public/popup/contact-shortcut.webp` (IA, Higgsfield, 720×900, 38 KB; las tarjetas se quitaron de la imagen a propósito para que sean HTML clicable y con esquinas vivas). Mide con `dataLayer` (`contact_popup_view|service|whatsapp|email|ai|close`) solo si GTM está cargado. Textos en `t.contactPopup`. Tests en `smoke.spec.ts` (usan `page.clock` para saltar el armado).

Navegación: la home YA NO tiene secciones ni anchors. Header, Footer y contenedores del juego (`data/port-containers.ts`) apuntan a rutas reales (`/projects`, `/resenas`, `/contact`, `/blog`). Un `href` que empiece por `#` en un contenedor es un destino sin página todavía (p. ej. `GALICIA`) y el juego lo ignora. `components/layout/SectionPage.tsx` = shell (Lenis + Header + Footer) para las páginas que antes eran secciones — la home no lo usa.
...
**Landings SEO** (`src/app/[landing]/page.tsx` + `src/data/landings.ts`; la home de mobile lista las mismas en `SERVICE_LANDINGS` de `packages/shared/src/seo.ts` vía `apps/mobile/src/components/SeoIntro.tsx` — **al añadir una landing, actualizar ambos sitios**, más `llms.txt`): NO se enlazan desde la navegación principal (decisión del cliente — no tocar el diseño original). Se descubren vía `sitemap.xml`, `/servicios`, `llms.txt` y enlazado entre ellas. Son server components estáticos, responsive, sin GSAP/Lenis. El middleware solo reescribe `/` a la zona mobile; el resto de rutas se sirven desde desktop en todos los dispositivos.
...
Componente nav: `layout/Header.tsx` + `Header.module.css` = **cápsula holográfica** compacta: NO ocupa el ancho de la pantalla, se ajusta a su contenido y queda centrada arriba (`mx-auto mt-3 w-fit`, 44px de alto; antes era un panel a todo el ancho de 64px y resultaba invasivo — decisión del cliente, no volver a estirarla). Así la franja superior queda libre: en la home los clicks del juego pasan al canvas fuera de la cápsula (el `<header>` sigue siendo `pointer-events-none` y solo la cápsula recibe eventos). Mismo lenguaje que la flecha holográfica del barco (`port/Ship.tsx` HOLO_FRAG) pero más apagado: lima translúcido con fondo `rgba(8,8,8,.30)` + blur, borde fresnel `#eaffb0`, barrido de franjas que sube (`holo-scan`), parpadeo con tartamudeo (`holo-flicker`), doble imagen desfasada (`::before`), haz difuminado abajo (`::after`) y corchetes en las esquinas. Sin degradado en la home (fuera, fundido corto de `h-20`). Marca con logo teñido en lima (30px) + telemetría mono en UNA sola línea (era la que forzaba los 64px): led, `VIGO`, hora local y fase del día vía `resolveTimeOfDay` + `?hora=` (solo en cliente), con las coordenadas 42.24°N 8.72°W únicamente a partir de `2xl`; enlaces con retícula al hover/activo; idioma segmentado; CTA "Hablemos" como prompt `>` con cursor que se solidifica en lima al hover.

**Persiana (carga + transición de ruta)** — `ui/Blinds.tsx` es la cortina: 10 lamas horizontales `bg-accent` con hueco de 3px, cierre 0→1 con origen arriba y apertura 1→0 con origen abajo, escalonadas de arriba abajo (transiciones CSS, sin GSAP; `blindsDuration()` devuelve 0 con `prefers-reduced-motion`). Dos consumidores: `ui/LoadingScreen.tsx` (cerrada desde SSR, se abre al llegar a 100 %) y `animations/PageTransition.tsx` (provider en el layout raíz). `PageTransition` intercepta en fase de CAPTURA cualquier `<a>` interno del documento — hay que ir por delante del `onClick` de `<Link>` de Next, que hace `preventDefault` y navega solo — cierra, hace `router.push`, y abre cuando cambia `location` (o a los 4 s como escotilla). Salta con modificadores, `target`, `download`, `data-no-transition`, externos y misma ruta. Navegación programática (juego de la home, logo del Header) → `usePageTransition().navigate(href)`. `navigate(href, { covered: true })` = el llamante ya pinta la persiana cerrada: se salta el cierre (`Blinds instant`) y solo anima la apertura — lo usa la puerta de `/projects`. `Blinds` usa las propiedades `transition*` sueltas, no el shorthand (React avisa al mezclarlo con `transitionDelay`). Estado observable en `[data-testid="page-blinds"][data-state]` para e2e. **Una sola cortina por navegación:** la home solo monta `LoadingScreen` si llega DESTAPADA (carga directa de `/`, recarga, back/forward). Si se aterriza por un link y la escena ya se cargó en esta sesión (`skipLoader` en `app/page.tsx` = `usePageTransition().busy` en el primer render + `sessionStorage`), la persiana de `PageTransition` ya está cubriendo y la de carga NO se monta: encadenar las dos hacía que volver a `/` tardase ~5 s frente a ~1,9 s de cualquier otra ruta. La escena 3D arranca detrás de la persiana de transición mientras se recoge y acaba de entrar con su `animate-fade-in`.
...
- **Home mobile indexable**: `apps/mobile/src/components/SeoIntro.tsx` (montado en `layout.tsx`) lleva el único `<h1>` de la zona mobile + enlaces a `/servicios` y las landings (`SERVICE_LANDINGS` de `packages/shared/src/seo.ts`), `sr-only` y visible con foco. El hero es un juego de palabras sueltas: esas van en `<p>` y el título de About en `<h2>` — **no volver a `<h1>`**. Lint de mobile = `eslint` con config plana (`apps/mobile/eslint.config.mjs`); `react-hooks/refs` solo apagada en `app/page.tsx`.
...
- **Blog + leads en Firebase** (proyecto `action-dev-1a531`) — única pieza con backend real. El resto sigue siendo datos estáticos en `src/data/`.
  ...
  - `apps/desktop`: SDK web de Firebase (`src/lib/firebase/client.ts`, solo `NEXT_PUBLIC_*`, sin sesión — las reglas ya limitan lo que puede hacer). `src/lib/blog.ts` (`getPosts`/`getPost`) lee `posts` directo; `src/lib/callback-request.ts` (`contact/CallbackForm.tsx`) hace `addDoc` en `leads`. `/blog` y `/blog/[slug]` llevan `revalidate = 3600` como red de seguridad; la publicación instantánea la dispara el webhook `POST /api/revalidate` (secreto compartido `REVALIDATE_SECRET`), llamado por las server actions de `apps/admin` tras cada guardado. `dynamicParams` es `true` (default) en `/blog/[slug]` — un slug nuevo se sirve on-demand aunque el webhook falle.
  - `apps/admin` (puerto 3003, **`basePath: "/admin"`**): se sirve en `actiondev.es/admin` — `apps/desktop` lo proxea con un `rewrites()` hacia `ADMIN_URL` (default `http://localhost:3003`; en prod, el deploy de Vercel de admin). Los `fetch` manuales del admin llevan el prefijo a mano (`/admin/api/session`); `Link`/`redirect`/`router.push` lo añaden solos. Panel de gestión, **Admin SDK** (`src/lib/firebase/admin.ts`), nunca el navegador. En local usa las credenciales de usuario de `firebase login` (`applicationDefault()`); en producción, una cuenta de servicio propia vía `FIREBASE_CLIENT_EMAIL`/`FIREBASE_PRIVATE_KEY`. Como el Admin SDK **ignora** `firestore.rules`, la única puerta es `requireSessionUser()`/`getSessionUser()` (`src/lib/firebase/session.ts`) — verifica la cookie de sesión contra el mismo allowlist de email. CRUD de posts vía server actions (`src/app/(protected)/posts/actions.ts`).
  ...
  - **Cuerpo de los posts**: `renderInline` (`app/blog/[slug]/page.tsx`) entiende `**negrita**` y enlaces internos `[texto](/ruta)` (solo `/` simple; `//…` y `https://…` se pintan como texto, a propósito). Los enlaces internos a las landings son la vía de enlazado SEO del blog.
