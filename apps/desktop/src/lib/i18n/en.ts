import type { Translations } from "./types";

export const en: Translations = {
  nav: {
    home: "Home",
    work: "Work",
    reviews: "Reviews",
    blog: "Blog",
    contact: "Contact",
    cta: "Let's talk",
    ariaLabel: "Main navigation",
    brandHome: "Action — Home",
  },
  hero: {
    label: "Digital Agency",
    headline1: "We craft digital",
    headline2: "experiences that",
    accent: "matter",
    sub: "Strategy, design, and development for brands that refuse to blend in.",
  },
  projects: {
    transform: "Development",
    into: "tailored",
    accent: "to your projects",
    loading: "Loading projects…",
    cargoTag: "Cargo",
  },
  testimonials: {
    trusted: "Trusted by",
    visionaries: "visionaries",
  },
  contact: {
    headline: "Message us",
    accent: "directly",
    subtitle: "No strings attached",
    whatsappLabel: "WhatsApp",
    emailLabel: "Email",
    emailSubject: "New project",
    intro: "Hi Action! I'd like to talk about a project.",
    askAI: "Ask AI about us",
    aiPrompt:
      "I want to understand what Action.dev is and what they do. They are a digital agency specializing in design and development, immersive web experiences with Three.js and React, brand identity, and growth services like SEO and CRO. Summarise their capabilities, notable work, and what makes them different: https://actiondev.es/",
    callbackLabel: "Prefer we reach out to you?",
    callbackPlaceholder: "Your phone number",
    callbackNotesPlaceholder: "Call or text? Any time that works best? Tell us anything else (optional)",
    callbackCta: "Send",
    callbackSuccess: "Thank you! We'll be in touch within 24h.",
    street: {
      address: "C/ Colón 20 · Vigo",
      loading: "Arriving at Colón 20…",
      hint: "Use the booth, the postbox or the intercom",
      whatsappTag: "WhatsApp",
      whatsappAction: "Pick up",
      emailTag: "Email",
      emailAction: "Post a letter",
      callbackTag: "We call you",
      callbackAction: "Ring the intercom",
      booth: "TELEPHONE",
      lcd1: "TALK TO ACTION",
      lcd2: "Pick up · WhatsApp",
      postbox: "LETTERS",
      collection: "COLLECTION",
      callbackClose: "Close",
    },
  },
  footer: {
    available: "Available for '26",
    tagline: "We craft digital experiences that matter. One project a month, no shortcuts.",
    workWithUs: "Got a project in mind?",
    workCTA: "Start a brief",
    sitemapTitle: "Sitemap",
    socialTitle: "Elsewhere",
    legalTitle: "Legal",
    legalNotice: "Legal notice",
    legalPrivacy: "Privacy",
    legalTerms: "Terms",
    legalCookies: "Cookies",
    cookiePreferences: "Cookie preferences",
    madeIn: "Designed & built in Vigo, ES",
    rights: "All rights reserved",
    brandDisclaimer:
      "{brand} is a trading name of {legal} · Spanish VAT {taxId}",
  },
  scroll: {
    hero: "Hero",
    work: "Work",
    reviews: "Reviews",
    blog: "Blog",
    contact: "Contact",
    goTo: "Go to",
    ariaLabel: "Section progress",
  },
  tutorial: {
    move: "move the crane",
    row: "change row",
    pickup: "hook a container",
    throw: "drop it on the ship",
  },
  loading: {
    ariaLabel: "Loading percentage",
  },
  game: {
    hud: {
      clickToGo: "Click to travel",
      soon: "Coming soon",
      hintRow: "Back to the ship's row",
      hintCarry: "Carry it to the ship's arrow",
      hintRelease: "Drop it! Press DROP",
      heading: "Heading to",
      skip: "Skip the game",
    },
    remote: {
      model: "ACTION RC-1",
      lower: "DROP",
      left: "Move the crane left",
      right: "Move the crane right",
      up: "Move the crane away",
      down: "Bring the crane closer",
      hook: "DROP SPACE — lowers the hook or releases the container",
      row: "ROW",
      actionKey: "SPACE",
    },
    tally: {
      label: "Score",
      timeLeft: "Time left",
      record: "Best",
      newRecord: "New record",
      seconds: "seconds",
      roundStart: "GULL HUNT!",
      roundEnd: "TIME!",
      streak: "STREAK",
      streakKills: "KILLS",
      alert: "ALERT!",
      alertSub: "GULLS GONE MAD",
    },
  },
  notFound: {
    subtitle: "The page you're looking for doesn't exist — or never did.",
    cta: "Back to home",
  },
  cookieConsent: {
    message:
      "We use analytics cookies to understand how the site is used. They only run if you accept them.",
    linkLabel: "Cookie policy",
    accept: "Accept",
    reject: "Reject",
    ariaLabel: "Cookie consent",
  },
  contactPopup: {
    eyebrow: "Shortcut · skip the game",
    title: "What do you want to build?",
    lead: "Pick one and message us. We reply within 24 h, no strings attached.",
    servicesLegend: "What you need",
    services: {
      apps: {
        label: "App development",
        message: "Hi Action! I'm interested in building an app. Shall we talk?",
      },
      webapp: {
        label: "Custom web app",
        message: "Hi Action! I need a custom web app. Shall we talk?",
      },
      web: {
        label: "Premium website",
        message: "Hi Action! I want a premium website for my business. Shall we talk?",
      },
    },
    whatsappCta: "Message us on WhatsApp",
    emailPrefix: "or by email:",
    reassurance: "Reply within 24 h · Team in Vigo · No forms",
    askAI: "Still unsure? Ask an AI",
    close: "Close",
  },
  plaza: {
    title: "The reviews plaza",
    subtitle: "Every client, a character. Every review, a real story.",
    googleCta: "See our Google reviews",
    hint: "Click a character",
    countLabel: "{count} reviews",
    loading: "Setting up the plaza…",
    closeAriaLabel: "Close review",
    dialogAriaLabel: "Review by {name}",
    ratingAriaLabel: "{rating} out of 5 stars",
    googleSource: "Google review",
  },
  arcade: {
    title: "Our work",
    loading: "Powering up the machines…",
    hintWalk: "Walk down the aisle",
    hintPick: "Pick a machine",
    door: "Let's work together",
    play: "View project",
    list: "View list",
    listTitle: "All projects",
    close: "Close list",
    screen: {
      back: "Back to the aisle",
      prev: "Machine on the left",
      next: "Machine on the right",
      neighbors: "Next machine",
      brief: "What they asked for",
      result: "What we delivered",
      site: "Visit site",
      caseStudy: "Full case study",
    },
  },
};
