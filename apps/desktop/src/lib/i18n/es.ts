import type { Translations } from "./types";

export const es: Translations = {
  nav: {
    home: "Inicio",
    work: "Trabajo",
    reviews: "Reseñas",
    blog: "Blog",
    contact: "Contacto",
    cta: "Hablemos",
    ariaLabel: "Navegación principal",
    brandHome: "Action — Inicio",
  },
  hero: {
    label: "Agencia Digital",
    headline1: "Creamos experiencias",
    headline2: "digitales que",
    accent: "importan",
    sub: "Estrategia, diseño y desarrollo para marcas que se niegan a pasar desapercibidas.",
  },
  projects: {
    transform: "Desarrollo",
    into: "a medida",
    accent: "para tus proyectos",
    loading: "Cargando proyectos…",
    cargoTag: "Carga",
  },
  testimonials: {
    trusted: "La confianza de",
    visionaries: "visionarios",
  },
  contact: {
    headline: "Escríbenos",
    accent: "directamente",
    subtitle: "Sin compromiso",
    whatsappLabel: "WhatsApp",
    emailLabel: "Email",
    emailSubject: "Proyecto nuevo",
    intro: "¡Hola Action! Me gustaría hablar de un proyecto.",
    askAI: "Pregunta a la IA sobre nosotros",
    aiPrompt:
      "Quiero entender qué es Action.dev y a qué se dedican. Son una agencia digital especializada en diseño y desarrollo, experiencias web inmersivas con Three.js y React, identidad de marca y servicios de crecimiento como SEO y CRO. Resume sus capacidades, trabajos destacados y qué les hace diferentes. Responde en español: https://actiondev.es/",
    callbackLabel: "¿Prefieres que te contactemos?",
    callbackPlaceholder: "Tu teléfono",
    callbackNotesPlaceholder:
      "¿Llamada o mensaje? ¿Algún horario mejor para llamarte? Cuéntanos lo que quieras (opcional)",
    callbackCta: "Enviar",
    callbackSuccess: "¡Gracias! Te contactaremos en menos de 24h.",
    street: {
      address: "C/ Colón 20 · Vigo",
      loading: "Llegando a Colón 20…",
      hint: "Usa la cabina, el buzón o el portero",
      whatsappTag: "WhatsApp",
      whatsappAction: "Descuelga",
      emailTag: "Email",
      emailAction: "Echa una carta",
      callbackTag: "Te llamamos",
      callbackAction: "Pulsa el portero",
      booth: "TELÉFONO",
      lcd1: "HABLA CON ACTION",
      lcd2: "Descuelga · WhatsApp",
      postbox: "CARTAS",
      collection: "RECOGIDA",
      callbackClose: "Cerrar",
    },
  },
  footer: {
    available: "Disponible para el '26",
    tagline: "Creamos experiencias digitales que importan. Un proyecto al mes, sin atajos.",
    workWithUs: "¿Tienes un proyecto en mente?",
    workCTA: "Cuéntanoslo",
    sitemapTitle: "Mapa del sitio",
    socialTitle: "En la red",
    legalTitle: "Legal",
    legalNotice: "Aviso legal",
    legalPrivacy: "Privacidad",
    legalTerms: "Términos",
    legalCookies: "Cookies",
    cookiePreferences: "Preferencias de cookies",
    madeIn: "Diseñado y desarrollado en Vigo, ES",
    rights: "Todos los derechos reservados",
    brandDisclaimer:
      "{brand} es una marca comercial de {legal} · CIF {taxId}",
  },
  scroll: {
    hero: "Inicio",
    work: "Trabajo",
    reviews: "Reseñas",
    blog: "Blog",
    contact: "Contacto",
    goTo: "Ir a",
    ariaLabel: "Progreso de sección",
  },
  tutorial: {
    move: "mueve la grúa",
    row: "cambia de fila",
    pickup: "engancha un contenedor",
    throw: "suéltalo en el barco",
  },
  loading: {
    ariaLabel: "Porcentaje de carga",
  },
  game: {
    hud: {
      clickToGo: "Clic para viajar",
      soon: "Próximamente",
      hintRow: "Vuelve a la fila del barco",
      hintCarry: "Llévalo hasta la flecha del barco",
      hintRelease: "¡Suéltalo! Pulsa BAJAR",
      heading: "Rumbo a",
      skip: "Ir sin jugar",
    },
    remote: {
      model: "ACTION RC-1",
      lower: "BAJAR",
      left: "Mover la grúa a la izquierda",
      right: "Mover la grúa a la derecha",
      up: "Alejar la grúa",
      down: "Acercar la grúa",
      // El nombre accesible CONTIENE el texto visible del botón ("BAJAR" +
      // la tecla impresa "ESPACIO"), como pide WCAG 2.5.3 (Label in Name):
      // axe compara contra lo que se VE, y la tecla se ve aunque vaya
      // `aria-hidden`. De paso, el atajo deja de ser información solo visual.
      hook: "BAJAR ESPACIO — baja el gancho o suelta el contenedor",
      row: "FILA",
      actionKey: "ESPACIO",
    },
    tally: {
      label: "Puntos",
      timeLeft: "Tiempo restante",
      record: "Récord",
      newRecord: "Nuevo récord",
      seconds: "segundos",
      roundStart: "¡RONDA DE CAZA!",
      roundEnd: "¡TIEMPO!",
      streak: "RACHA",
      streakKills: "MUERTES",
      alert: "¡ALERTA!",
      alertSub: "GAVIOTAS ENFURECIDAS",
    },
  },
  notFound: {
    subtitle: "La página que buscas no existe — o nunca existió.",
    cta: "Volver al inicio",
  },
  cookieConsent: {
    message:
      "Usamos cookies de analítica para entender cómo se usa el sitio. Solo se activan si las aceptas.",
    linkLabel: "Política de cookies",
    accept: "Aceptar",
    reject: "Rechazar",
    ariaLabel: "Consentimiento de cookies",
  },
  contactPopup: {
    eyebrow: "Atajo · sin jugar",
    title: "¿Qué quieres construir?",
    lead: "Elige una opción y escríbenos. Te respondemos en menos de 24 h, sin compromiso.",
    servicesLegend: "Qué necesitas",
    services: {
      apps: {
        label: "Desarrollo de apps",
        message: "¡Hola Action! Me interesa desarrollar una app. ¿Hablamos?",
      },
      webapp: {
        label: "Web app a medida",
        message: "¡Hola Action! Necesito una web app a medida. ¿Hablamos?",
      },
      web: {
        label: "Web premium",
        message: "¡Hola Action! Quiero una web premium para mi negocio. ¿Hablamos?",
      },
    },
    whatsappCta: "Escribir por WhatsApp",
    emailPrefix: "o por email:",
    reassurance: "Respuesta en < 24 h · Equipo en Vigo · Sin formularios",
    askAI: "¿Aún con dudas? Pregunta a la IA",
    close: "Cerrar",
  },
  plaza: {
    title: "La plaza de las reseñas",
    subtitle: "Cada cliente, un personaje. Cada reseña, una historia real.",
    googleCta: "Ver reseñas en Google",
    hint: "Haz click en un personaje",
    countLabel: "{count} reseñas",
    loading: "Preparando la plaza…",
    closeAriaLabel: "Cerrar reseña",
    dialogAriaLabel: "Reseña de {name}",
    ratingAriaLabel: "{rating} de 5 estrellas",
    googleSource: "Reseña de Google",
  },
  arcade: {
    title: "Nuestros trabajos",
    loading: "Encendiendo las máquinas…",
    hintWalk: "Avanza por el pasillo",
    hintPick: "Elige una máquina",
    door: "Trabajemos juntos",
    play: "Ver proyecto",
    list: "Ver lista",
    listTitle: "Todos los proyectos",
    close: "Cerrar lista",
    screen: {
      back: "Volver al pasillo",
      prev: "Máquina de la izquierda",
      next: "Máquina de la derecha",
      neighbors: "Máquina de al lado",
      brief: "Qué nos pidieron",
      result: "Qué conseguimos",
      site: "Ver web",
      caseStudy: "Ficha completa",
    },
  },
};
